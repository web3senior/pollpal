...
export default function Search({ title }) {
  ...
  const Days = ({from, to}) => {
    ...
    for (let i = from - 1; i <= to; i++) {
      days.push(<option value={i+1}>{i+1}</option>)
    }
    return days
  }
  ...
  return (
    <section className={`${styles.section}  ms-motion-slideUpIn p-relative d-f-c`}>
      <div className={`${styles['container']} __container d-flex flex-column align-items-center justify-content-center`} data-width={`medium`}>
        <div className={`${styles['pageTitle']}`}>
          {title} <span>Poll</span>
        </div>

        <div className={`card ms-depth-16 w-100`} data-hover={`none`}>
          <div className={`card__body ${styles.form}`}>
            <form action="" className={`w-100 d-flex flex-column align-items-center justify-content-between`}>
              ...
              <input type={`text`} name={``} id={``} placeholder={`Choice 1`} />
              <input type={`text`} name={``} id={``} placeholder={`Choice 2`} />
              <input type={`text`} name={``} id={``} placeholder={`Choice 3 (Optional)`} />
              <input type={`text`} name={``} id={``} placeholder={`Choice 4 (Optional)`} />

              <ul className={`${styles['selectBox']} d-flex align-items-center justify-content-center w-100`}>
                <li>
                ...
                <li>
                  <label htmlFor={`days`}>Minutes</label>
                  <select name="" id={`days`}>
                    <Days  from={0}  to={60}/>
                  </select>
                </li>
              ...
              <div>
                <b>Everyone can vote</b>
              </div>

              <div className={`${styles['profile-container']}`}></div>
              <input type="text" name="" id="" placeholder="Enter address of recipient" />
